...
class TestGNMIcCollector:
    ...
    def test_gnmic_metrics_endpoint(self, gnmic_url):
        ...
        try:
            ...
            print(f"✓ gNMIc metrics endpoint accessible")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('status') == 'success':
                    ...
                    gnmic_targets = [t for t in targets if 'gnmic' in t.get('labels', {}).get('job', '')]
    ...
    def test_grafana_prometheus_datasource(self, grafana_url, wait_for_grafana):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if prom_datasources:
                    print(f"✓ Prometheus datasource configured: {len(prom_datasources)} datasource(s)")
...
class TestEndToEndPipeline:
    ...
    def test_metric_flow_gnmic_to_prometheus(self, gnmic_url, prometheus_url, 
                                             wait_for_prometheus, lab_deployed):
        """
                ...
                for metric in sample_metrics:
                    ...
                    if prom_response.status_code == 200:
                        ...
                        if data.get('status') == 'success':
                            results = data.get('data', {}).get('result', [])
                            if results:
                                print(f"✓ Metric '{metric}' flowing to Prometheus ({len(results)} series)")
                            else:
    ...
    def test_dashboard_queries_work(self, grafana_url, prometheus_url, 
                                   wait_for_grafana, wait_for_prometheus):
        """
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data.get('status') == 'success':
                        results = data.get('data', {}).get('result', [])
                        if results:
                            print(f"✓ Dashboard query '{query_name}' returns data ({len(results)} series)")
                        else:
                            print(f"⚠ Dashboard query '{query_name}' returns no data (may be too early)")
                    else:
