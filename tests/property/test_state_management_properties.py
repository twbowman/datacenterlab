...
from .strategies import topologies, configurations
...
def test_lab_state_round_trip(topology, lab_name):
    ...
    **Validates: Requirements 12.2**
    ...
    # Restore lab state
    restored_lab_name, restored_topology, restored_configurations = state_manager.restore_state(snapshot)
    
    # Property: Lab name must be preserved
    assert restored_lab_name == lab_name, \
        f"Lab name changed: {lab_name} -> {restored_lab_name}"
    
    # Property: Topology must be equivalent
    assert restored_topology == topology, \
        "Topology changed during export/restore cycle"
