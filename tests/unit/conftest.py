...
def sample_topology():
    ...
    return {
        ...
        "topology": {
            ...
            "links": [{"endpoints": ["spine1:e1-1", "leaf1:e1-49"]}],
        },
    }
...
def sample_bgp_config():
    ...
    return {
        ...
        "router_id": "10.0.0.1",
        "neighbors": [
            {"ip": "10.1.1.1", "peer_as": 65011, "group": "leafs", "description": "to leaf1"}
        ],
    }
...
def sample_validation_report():
    ...
    return {
        ...
        "device": "spine1",
        "checks": {
            "bgp_sessions": {"status": "pass", "expected": 4, "actual": 4, "details": []},
            "interface_states": {"status": "pass", "expected": 8, "actual": 8, "details": []},
        },
        "overall_status": "pass",
        "duration_seconds": 12,
    }
...
def sample_lab_state():
    ...
    return {
        ...
        "topology": {
            "name": "test-lab",
            "nodes": {"spine1": {"kind": "nokia_srlinux", "image": "ghcr.io/nokia/srlinux:latest"}},
        },
        "configurations": {
...
def mock_file_operations(monkeypatch, mock_topology_file):
    ...
    return {"exists": mock_exists, "read_text": mock_read_text, "open": mock_open_func}
...
def mock_subprocess(monkeypatch):
    ...
    return {"run": mock_run, "popen": mock_popen, "result": mock_result}
...
def mock_docker_commands(mock_subprocess):
    ...
    def mock_docker_command(command, *args, **kwargs):
        if "ps" in command:
            ...
            mock_subprocess["result"].stdout = json.dumps(
                [{"State": {"Running": True}, "NetworkSettings": {"IPAddress": "172.20.20.10"}}]
            )
        return mock_subprocess["result"]
...
def mock_gnmi_connection():
    ...
    # Mock get
    def mock_get(path):
        return {"notification": [{"update": [{"path": path, "val": {"state": "up"}}]}]}
    ...
    def mock_exec_command(command):
        ...
        if "show bgp" in command:
            stdout.read.return_value = (
                b"BGP neighbor is 10.1.1.1, remote AS 65011, state Established"
            )
