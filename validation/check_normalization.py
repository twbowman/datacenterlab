...
class MetricNormalizationValidator:
    ...
    def check_prometheus_connectivity(self) -> bool:
        ...
        try:
            ...
            self.checks.append(
                NormalizationCheck(
                    name="Prometheus Connectivity", passed=False, message=f"Connection failed: {e}"
                )
            )
            return False
    ...
    def check_normalized_metrics_exist(self) -> bool:
        ...
        self.checks.append(
            NormalizationCheck(
                name="Normalized Metrics Exist", passed=passed, message=message, details=details
            )
        )
        return passed
    ...
    def check_vendor_coverage(self) -> bool:
        ...
        self.checks.append(
            NormalizationCheck(
                name="Vendor Coverage", passed=passed, message=message, details=details
            )
        )
        return passed
    ...
    def check_metric_values_preserved(self) -> bool:
        ...
        for metric in self.EXPECTED_METRICS:
            ...
            for result in data["result"]:
                ...
                try:
                    ...
                    issues.append(
                        f"{metric_name} from {vendor}/{source}: non-numeric value '{value}'"
                    )

        passed = len(issues) == 0 and checked_metrics > 0

        details = {"checked_metrics": checked_metrics, "issues": issues}
        ...
        self.checks.append(
            NormalizationCheck(
                name="Metric Values Preserved", passed=passed, message=message, details=details
            )
        )
        return passed
    ...
    def check_interface_name_normalization(self) -> bool:
        ...
        self.checks.append(
            NormalizationCheck(
                name="Interface Name Normalization", passed=passed, message=message, details=details
            )
        )
        return passed
    ...
    def check_cross_vendor_consistency(self) -> bool:
        ...
        details = {"all_vendors": list(all_vendors), "inconsistent_metrics": inconsistent_metrics}
        ...
        self.checks.append(
            NormalizationCheck(
                name="Cross-Vendor Consistency", passed=passed, message=message, details=details
            )
        )
        return passed
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Validate metric normalization in Prometheus")
    parser.add_argument(
        "--prometheus-url",
