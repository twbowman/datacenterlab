...
class DeviceOSDetector:
    ...
    def __init__(self, topology_file='topology.yml'):
    ...
    def get_gnmi_capabilities(self, host, port=57400, username='admin', password='NokiaSrl1!'):
        """
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(f"Warning: Failed to get capabilities from {host}: {result.stderr}", file=sys.stderr)
                return None
    ...
    def detect_os_from_kind(self, node_config, groups):
        ...
        if 'group' in node_config and groups:
            ...
            if group_name in groups:
                kind = groups[group_name].get('kind')
            else:
                kind = None
        else:
    ...
    def detect_device_os(self, node_name, node_config, groups):
        ...
        if not mgmt_ip:
            print(f"Warning: No management IP for {node_name}, using fallback detection", file=sys.stderr)
        else:
            ...
            if detected_os != 'unknown':
                print(f"Detected {node_name} as {detected_os} via gNMI capabilities", file=sys.stderr)
                return detected_os
    ...
    def generate_inventory(self):
        ...
        for node_name, node_config in nodes.items():
            ...
            if detected_os not in os_groups:
                os_groups[detected_os] = {
                    'hosts': {},
                    'vars': self._get_os_vars(detected_os)
                }
...
def main():
    ...
    parser.add_argument(
        ...
        help='Path to containerlab topology file (default: topology.yml)'
    )
    parser.add_argument(
        '-o', '--output',
