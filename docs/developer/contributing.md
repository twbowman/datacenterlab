...
Pre-commit (local, runs before every commit):
  ...
  Security: detect-private-key, gitleaks, bandit
```
...
**Local Security:**
6. `detect-private-key` — blocks commits containing private keys (PEM blocks)
7. `check-added-large-files` — prevents files >500KB (may contain embedded secrets/data)
8. `gitleaks` — scans staged changes for secrets (AWS, Azure, GCP, 150+ patterns) using `.gitleaks.toml` rules
9. `bandit` — Python security analysis (medium+ severity, scoped to `scripts/` and plugins)
