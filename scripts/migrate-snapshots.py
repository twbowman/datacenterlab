    ...
    try:
        ...
        if validate:
            ...
            if not is_valid:
                ...
                return False
            print(f"✓ Input snapshot is valid")
        ...
        if validate:
            ...
            if not is_valid:
                ...
                return False
            print(f"✓ Migrated snapshot is valid")
        ...
        if not dry_run:
            ...
            print(f"✓ Dry run complete (no file written)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Migrate state snapshots between format versions"
    )
    parser.add_argument(
        '--from',
        dest='from_version',
        required=True,
        help='Source version (e.g., 1.0)'
    )
    parser.add_argument(
        '--to',
        dest='to_version',
        required=True,
        help='Target version (e.g., 2.0)'
    )
    parser.add_argument(
        '--input',
        required=True,
    ...
    parser.add_argument(
        '--output',
        help='Output snapshot file or directory (default: <input>-migrated)'
    )
    parser.add_argument(
        '--validate',
        action='store_true',
    ...
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Perform migration without writing output files'
    )
    ...
    if input_path.is_dir():
        ...
        output_dir = Path(args.output) if args.output else input_path.parent / f"{input_path.name}-migrated"
        ...
        # Migrate single file
        output_path = args.output if args.output else str(input_path.parent / f"{input_path.stem}-migrated{input_path.suffix}")
