...
def query_prometheus(query, time_range="5m"):  # noqa: ARG001
...
def print_report(utilization, issues):
    ...
    print("Analysis Period: Last 5 minutes")
    print()
                ...
                for issue in severity_issues:
                    if issue["type"] == "imbalance":
                        ...
                        for link in issue["links"]:
                            print(
                                f"     - {link['device']}: {link['percent']:.1f}% ({link['gbps']:.2f} Gbps)"
                            )
                    else:
                        print(f"\n  • {issue['device']} - {issue['interface']}")
                        print(f"    {issue['message']}")
                        print(
                            f"    Current: {issue['gbps']:.2f} Gbps ({issue['utilization']:.1f}%)"
                        )
    else:
